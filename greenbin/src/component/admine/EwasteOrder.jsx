...
const EwasteOrder = () => {
    ...
    const { handleConfirmOrderEwaste } = useContext(Context);

    // Fetch data from the API
    const fetchOrderDetails = async () => {
        try {
            const response = await api.get("/selling/ewaste");
    ...
    const handleConfirmClick = async (status, orderId) => {
        await handleConfirmOrderEwaste(status, orderId);
